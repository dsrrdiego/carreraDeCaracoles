...
if (largoInt<=1.4 || largoInt >=4){achicamiento=achicamiento*-1;};
    }else{
    y+=masY;
    x+=masX;
    glPushMatrix();
...
void check(){
//    FLOAT PIX;
//glReadPixels(0,0,1,1,GL_ALPHA,GL_FLOAT,PIX);
        ...
        for (int b=0;b<=nroDeJugadores;b++){
                ...
                if (
                    ...
                    //caracol[a].x=0;
                    {
                       ...
                       /*or(int babaABorrar=0;babaABorrar == caracol[a].largo;babaABorrar++){
                            baba[a][babaABorrar].x = 999999;
                            baba[a][babaABorrar].y = 999999;
                       */
...
glOrtho(-120,120,-120,120,-10,170);
    ...
    gluLookAt(20,60,45,0,0,0,0,0,1);
    //glLoadIdentity() ;
}
int cadacien=10;//caracol[0].velocidad*50;
...
case GLUT_KEY_RIGHT: tecl[3]=true;//caracol[1].angulo-=angulacion;
break;
case GLUT_KEY_LEFT: tecl[2]=true;//racol[1].angulo+=angulacion;
break;
}}
...
//for (int a=0;a<=caracol[0].largo,a++){
...
break;
case GLUT_KEY_LEFT: tecl[2]=false;
break;
...
void teclas(){
//std::cout << tecl[0];
if (tecl[0]) caracol[0].angulo+=angulacion;
if (tecl[1]) caracol[0].angulo-=angulacion;
if (tecl[2]) caracol[1].angulo+=angulacion;
if (tecl[3]) caracol[1].angulo-=angulacion;
if (tecl[4]) caracol[2].angulo+=angulacion;
if (tecl[5]) caracol[2].angulo-=angulacion;
if (tecl[6]) caracol[3].angulo+=angulacion;
if (tecl[7]) caracol[3].angulo-=angulacion;
};
...
// char *nombre= itoa(22);//itoa(caracol[aa].puntos);

 glColor3f(1, 0, 0.1);
 sprintf(label,"%d",  caracol[aa].puntos);
  glRasterPos2f(50-aa*30, -100);
 drawString (label);
...
//gluLookAt(20,20,20,0,,0,0,0,1);
    ...
    if (cadacien==0){
        for (int b=0;b<=nroDeJugadores;b++){
            ...
            caracol[b].NroDeBaba++;
            if (caracol[b].NroDeBaba==caracol[b].largo) caracol[b].NroDeBaba=0;
        };
